...
class Reinforce(object):
    ...
    def __init__(self, policy_net: ANN, optimizer: torch.optim,  gamma: float=0.99, standardize_returns: bool=True, entropy_coef: float=0., action_space: str=Literal["continuous", "discrete"], max_action: float=np.float32('inf'), min_action: float=-np.float32('inf'), sample_action: bool=True, seed: int=42) -> None:
        self.policy_net = policy_net
        self.optimizer = optimizer
        self.memory = Memory('on-policy', custom_fields={"logprob_actions": None, "entropy": None})
        self.gamma = gamma
    ...
    def update(self, reward: float, logprob_action: torch.tensor, done: bool, entropy: torch.tensor=None) -> None:
        """ Updates the memory with the required quantities of the new experience """
        self.memory.update(reward=reward, logprob_actions=logprob_action, done=done, entropy=entropy)
    ...
    def train(self) -> float:
        ...
        for ep in range(num_eps):
            ...
            assert len(logprob_actions) == len(rewards), "Length of quantities inside episode do not match"
            len_eps = len(rewards)
            ...
            traj_returns = torch.tensor(traj_returns, dtype=self.dtype, device=self.device)
            
            if self.standardize_returns:
                traj_returns = (traj_returns - traj_returns.mean()) / (traj_returns.std() + self.eps)
...
if __name__ == "__main__":
    ...
    # define agent
    policy_net = ANN(num_inputs=in_dim, num_outputs=2, hidden_layer_dims=[64, 32], softmax_output=False, dropout=0., use_batch_norm=False, weight_init="xavier_normal", weight_init_kw={"gain": 1.0})
    optimizer = optim.Adam(params=policy_net.parameters(), lr=1e-2)
    policy = Reinforce(policy_net=policy_net, optimizer=optimizer, gamma=gamma, entropy_coef=entropy_coef, action_space="discrete", sample_action=sample_action, seed=seed)
    ...
    for ep in range(1, num_eps+1):
        ...
        for t in range(maxlen_ep):
            ...
            policy.update(reward=reward, logprob_action=log_a, done=done, entropy=entropy)
        ...
        # train
        if ep % train_freq == 0:
            total_reward = np.mean(list(map(lambda x: sum(x), policy.memory.rewards[:-1])))
            if total_reward > success_thres:
                print(f"Congratulations total reward of {total_reward:.2f}, environment has been solved successfully!")
                break
            loss = policy.train()
            print(f"Episode: {ep}, Total reward: {total_reward:.2f}, Training loss: {loss:.2f}")
    env.close() 
