...
class Optimizer(object):
    def __init__(self, model,  type: str="Adam", lr: float=0.001, optimizer_kw: Dict={}) -> None:
        self.type = type
        self.lr = lr
        self.optimizer = eval(f"torch.optim.{self.type}(model.parameters(), lr={self.lr}, **optimizer_kw)")
...
if __name__ == "__main__":
    ...
    # define optimizer
    optimizer = Optimizer(net, "SGD", 0.1, {"momentum": 0.9, "weight_decay": 1e-4})
    
    # forward-pass
    x = torch.randn((batch, num_inputs), device="cpu", dtype=torch.float32) # batch x dim
    y = net(x)
    loss = torch.mean((1. - y)**2)
