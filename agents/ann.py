...
class ANN(nn.Module):
    ...
    def __init__(
        ...
        activation_function_kw: Dict = {},
        softmax_output: bool = False,
        dropout: float = 0.0,
        use_batch_norm: bool = False,
        weight_init: str = "kaiming_uniform",
        weight_init_kw: Dict = {"a": np.sqrt(5)},
        dtype=torch.float32,
        seed: int = 42,
    ) -> None:
    ...
    def _define_layers(self) -> None:
        ...
        for k, current in enumerate(self.hidden_layer_dims):
            setattr(
                self,
                f"fc{k+1}",
                nn.Linear(prev, current, device=self.device, dtype=self.dtype),
            )
            ...
            setattr(
                self,
                f"activation{k+1}",
                eval(f"nn.{self.activation_fun}(**self.activation_fun_kw)"),
            )
            ...
            if self.use_batch_norm:
                setattr(
                    ...
                    f"bn{k+1}",
                    nn.BatchNorm1d(
                        current, device=self.device, dtype=self.dtype
                    ),
                )
        ...
        # output layer
        self.out = nn.Linear(
            current, self.num_outputs, device=self.device, dtype=self.dtype
        )
    ...
    def _init_weights(self, layer) -> None:
        eval(
            f"nn.init.{self.weight_init}_(layer.weight, **self.weight_init_kw)"
        )
